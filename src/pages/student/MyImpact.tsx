...
export default function MyImpact() {
  ...
  // Fetch impacts when selected year group changes
  useEffect(() => {
    if (selectedYearGroupForImpacts) {
      void (dispatch as any)(
        fetchStudentImpacts(selectedYearGroupForImpacts.id)
      );
    }
  }, [dispatch, selectedYearGroupForImpacts]);
  ...
  // show the success message when the impact is added, or deleted (since its sent for moderation)
  useEffect(() => {
    if (message) {
  ...
  const handleWritingSubmit = () => {
    if (!writingTitle || !writingContent || !selectedYearGroupForImpacts)
      return;
    ...
    void (dispatch as any)(
      createStudentImpact({
        ...
        yearGroupId: selectedYearGroupForImpacts.id,
      })
    )
      .unwrap()
      .then(() => {
      ...
      .catch((error: unknown) => {
        const errorMessage =
          error instanceof Error ? error.message : "Failed to create impact";
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      // Create impact with uploaded file URL
  ...
  const removeContentItem = (id: number) => {
    ...
    void (dispatch as any)(deleteStudentImpact(id))
      .unwrap()
      .then(() => {
        if (selectedYearGroupForImpacts) {
          void (dispatch as any)(
      ...
      .catch((error: unknown) => {
        const errorMessage =
          error instanceof Error ? error.message : "Failed to delete impact";
        toast.error(errorMessage);
      });
  };

  const handleViewImpact = (impact: any) => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Please select a year group from the sidebar to view your impacts.
          </p>
          <Button asChild>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading your impacts...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-blue-50 to-purple-50 flex items-center justify-center">
        <div className="text-center">
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
            <p className="font-bold">Error loading impacts</p>
    ...
    <div className="min-h-screen ">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-orange-100 mt-1">
                {selectedYearGroupForImpacts?.name || "Select a year group"}
              </p>
            </div>
          </div>
        </div>
                  ...
                  >
                    ...
                    <DialogContent className="!max-w-7xl">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Share how you've made a positive difference
                        </DialogDescription>
                      </DialogHeader>
                    ...
                    <DialogContent className="max-w-2xl">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <Input
                            id="upload-title"
                            placeholder="What impact are you showcasing?"
                            value={uploadTitle}
              ...
              <CardContent className="pt-6 text-center">
                <div className="text-gray-500">
                  ...
                  <p className="text-xl font-medium mb-2 text-gray-700">
                    No impacts yet for {selectedYearGroupForImpacts?.name}
                  </p>
                ...
                return (
                  <Card
                    ...
                    className={`relative ${colors.bg} ${
                      ...
                      impact.status === "rejected" ? "opacity-60" : ""
                    } ${
                      impact.status === "pending_deletion" ? "opacity-75" : ""
                    }`}
                    onClick={() => handleViewImpact(impact)}
                  >
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            removeContentItem(impact.id);
                          }}
                          disabled={
                            isSubmitting ||
                            impact.status === "pending_deletion" ||
                        ...
                        >
                          {isDeleting && itemBeingDeleted === impact.id ? (
                            <Spinner size="sm" />
                          ) : (
                            <X className="h-3 w-3" />
                          )}
                        ...
                        >
                          {(() => {
                            try {
                              ...
                              return impact.description || "No description";
                            } catch {
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogContent className="!max-w-7xl">
          <DialogHeader>
            <DialogTitle className="text-2xl text-pink-700">
              {selectedImpact?.title || "Impact"}
            </DialogTitle>
            <DialogDescription>View your impact details</DialogDescription>
          </DialogHeader>
          {selectedImpact && (
            <div className="space-y-4">
              <div>
                ...
                <div className="p-4 bg-gray-50 rounded-md border border-gray-200 min-h-[200px]">
                  {(() => {
                    try {
                      // Try to parse as JSON (TipTap content)
                      const parsed =
                        typeof selectedImpact.description === "string"
                          ? JSON.parse(selectedImpact.description)
                      ...
                      return (
                        <p className="text-gray-700 whitespace-pre-wrap">
                          {selectedImpact.description || "No description"}
                        </p>
                      );
                    } catch {
                      // If parsing fails, treat as plain text
              ...
              {selectedImpact.attachment_url && (
                <div>
                  ...
                  <div className="p-3 bg-gray-50 rounded-md border border-gray-200">
                    <a
                      href={selectedImpact.attachment_url}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      ...
                      {selectedImpact.attachment_url.split("/").pop()}
                    </a>
                  </div>
                </div>
              )}
              <div className="flex items-center gap-2 text-sm text-gray-500">
                ...
                <span>
                  Created:{" "}
                  {new Date(selectedImpact.created_at).toLocaleDateString()}
                </span>
